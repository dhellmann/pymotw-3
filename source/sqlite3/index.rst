...
``connect()`` so it knows when to create the schema for new
databases.
...
The ``executescript()`` method of the ``Connection`` can be
...
To retrieve the values saved in the ``task`` table from within a
Python program, create a ``Cursor`` from a database connection.
...
cursor's ``execute()`` method to tell the database engine what data
to collect.  Then, use ``fetchall()`` to retrieve the results.  The
...
The results can be retrieved one at a time with ``fetchone()``, or
...
The value passed to ``fetchmany()`` is the maximum number of items
...
called, the ``Cursor`` should set its :attr:`description`
...
``Connection`` objects have a ``row_factory`` property that
allows the calling code to control the type of object created to
represent each row in the query result set.  ``sqlite3`` also
includes a ``Row`` class intended to be used as a row factory.
Column values can be accessed through ``Row`` instances by using
...
re-written using ``Row`` instances instead of tuples.  The row
...
variables* passed to ``execute()`` along with the SQL instruction.
...
``id`` value is used to find the right row to modify, and the
``status`` value is written to the table.
...
``str``, unless the :attr:`text_factory` for the
...
``PARSE_DECLTYPES`` if the column was declared using the desired
...
using the classes ``date`` and ``datetime`` from the
...
Use the ``detect_types`` flag ``PARSE_COLNAMES`` when the
...
``commit()``.  This requirement gives an application an opportunity
...
The effect of calling ``commit()`` can be seen with a program that
...
When ``show_projects()`` is called before ``conn1`` has been
committed, the results depend on which connection is used.  Since the
change was made through ``conn1``, it sees the altered data.
However, ``conn2`` does not.  After committing, the new connection
...
``rollback()``.  The ``commit()`` and ``rollback()`` methods are
...
After calling ``rollback()``, the changes to the database are no
longer present.
...
:mod:`threading` module.  The ``writer()`` function connects and
make changes to the database, but does not commit before the event
fires.  The ``reader()`` function connects, then waits to query the
...
second writer block until it commits.  The ``sleep()`` call
...
``execute()`` call is committed immediately when the statement
...
In ``sqlite3_autocommit.py``, the explicit call to ``commit()`` has
...
filename when creating the ``Connection``.  Each ``':memory:'``
...
``iterdump()`` method of the ``Connection``.  The iterator
returned by ``iterdump()`` produces a series of strings that
...
``iterdump()`` can also be used with databases saved to files, but
...
``Connection``.  The parameters are the name of the function (as
...
with SQL syntax. For example, a function ``regexp`` can be used in a
...
This examples associates a function with ``regexp()`` to test values
using Python's :mod:`re` module.

.. literalinclude:: sqlite3_regex.py
   :caption:
   :start-after: #end_pymotw_header
...
.. {{{cog
.. cog.out(run_script(cog.inFile, 'sqlite3_regex.py'))
.. }}}
...
are ``avg()`` (average), ``min()``, ``max()``, and
``count()``.

The API for aggregators used by ``sqlite3`` is defined in terms of
a class with two methods.  The ``step()`` method is called once for
...
method of the ``Connection``.  The parameters are the name of the
function (as it should be used from within SQL), the number of
arguments the ``step()`` method takes, and the class to use.
...
``Connection`` objects cannot be shared between threads.  Each
...
on the action code.  For ``SQLITE_READ`` operations, the
...
by returning ``SQLITE_DENY``, which in turn causes SQLite to
raise an exception.
