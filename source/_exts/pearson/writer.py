...
\%(doctype)s{%(title)s}\label{%(title)s}
...
class PearsonLaTeXWriter(writers.Writer):
...
class PearsonLaTeXTranslator(nodes.NodeVisitor):
    ...
    def __init__(self, document, builder):
        ...
        # we always treat chapter files as chapters, so any other
        # sections we find in the input should start as "section"
        # level
        self.top_sectionlevel = self.sectionnames.index('section')
        ...
        self.elements.update({
            ...
            'indexname':    _('Index'),
            'doctype':      document.settings.doctype,
        })
    ...
    def visit_collected_footnote(self, node):
        ...
        if 'footnotetext' in node:
            self.body.append('\\footnotetext[%s]{\n' % node['number'])
        else:
            self.body.append('\\footnote[%s]{\n' % node['number'])
