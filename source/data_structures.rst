...
``Deque`` is a double-ended queue, and allows the addition or
...
added to it.  And ``namedtuple`` extends the normal
...
their contents, including recursive copies with ``deepcopy()``.
